...
function processSum(numberList, callback) {
  const reducer = (sum, nextNum) => {
    return sum + nextNum;
  };
  const sum = numberList.reduce(reducer, 0);  
return callback(sum);
}
...
function processDuplicateFree(list, callback) {
  let dedupList = [];
  list.forEach(element => { //function for foreach loop
    if (!dedupList.includes(element)){
      dedupList.push(element);
    }
  }); //close function for foreach loop and close (variable)
  return dedupList; 
}
...
function firstNamesAllCaps(runners) {
  let firstNamesCaps =[];
  runners.forEach(runner => {
    firstNamesCaps.push(runner.first_name.toUpperCase());
  });
  return firstNamesCaps;
}
